...
public class MfSDKHelper {
    ...
    public interface MFSDKHelper extends IMyfatoorahModule {
        ...
        static void setup(@NonNull BinaryMessenger binaryMessenger, @Nullable MFSDKHelper api) {
            ...
            {
                BasicMessageChannel<Object> channel =
                        new BasicMessageChannel<>(binaryMessenger, MFConstants.ChannelName.executePaymentWithGooglePay, getCodec());
                if (api != null) {
                    channel.setMessageHandler(
                            (message, reply) -> {
                                ArrayList<Object> args = (ArrayList<Object>) message;
                                MFExecutePaymentRequest executePaymentRequest = (MFExecutePaymentRequest) args.get(0);
                                String lang = (String) args.get(1);
                                api.ExecutePaymentWithGooglePay(executePaymentRequest, lang, resultCallback(reply));
                            });
                } else {
                    channel.setMessageHandler(null);
                }
            ...
            {
                BasicMessageChannel<Object> channel =
                        new BasicMessageChannel<>(binaryMessenger, MFConstants.ChannelName.updateSession, getCodec());
                if (api != null) {
                    channel.setMessageHandler(
                            (message, reply) -> {
                                ArrayList<Object> args = (ArrayList<Object>) message;
                                MFUpdateSessionRequest updateSessionRequest = (MFUpdateSessionRequest) args.get(0);
                                api.UpdateSession(updateSessionRequest, resultCallback(reply));
                            });
                } else {
                    channel.setMessageHandler(null);
                }
            ...
            {
                BasicMessageChannel<Object> channel =
                        new BasicMessageChannel<>(binaryMessenger, MFConstants.ChannelName.googlePayToken, getCodec());
                if (api != null) {
                    channel.setMessageHandler(
                            (message, reply) -> {
                                ArrayList<Object> args = (ArrayList<Object>) message;
                                ...
                                api.SetupGooglePayTokenHelper(googlePayRequest, resultCallback(reply));
                            });
                } else {
                    channel.setMessageHandler(null);
                }
        ...
        static <T> IMFCallBack<T> resultCallback(BasicMessageChannel.Reply<Object> reply) {
            return new IMFCallBack<T>() {
                private final AtomicBoolean hasReplied = new AtomicBoolean(false);

                @Override
                public void success(T result) {
                    if (hasReplied.getAndSet(true)) {
                        Log.w("MyFatoorahFlutterPlugin", "! Duplicate success() ignored. Reply already sent.");
                        return;
                    }; // already replied
                ...
                public void error(MFError error) {
                    if (hasReplied.getAndSet(true)) {
                        Log.w("MyFatoorahFlutterPlugin", "! Duplicate error() ignored. Reply already sent.");
                        return;
                    }; // already replied
