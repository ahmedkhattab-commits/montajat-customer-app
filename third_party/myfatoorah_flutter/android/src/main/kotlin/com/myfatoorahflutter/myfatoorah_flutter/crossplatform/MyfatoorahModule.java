...
public class MyfatoorahModule implements MfSDKHelper.MFSDKHelper {
    ...
    public MFGooglePayTokenHelper mfGooglePayTokenHelper = null;
    ...
    @NonNull
    @Override
    public void ExecutePaymentWithGooglePay(@NonNull MFExecutePaymentRequest executePaymentRequest, @NonNull String lang, @NonNull IMFCallBack promise) {
    ...
    public void Load(MFInitiateSessionResponse initiateSessionResponse, boolean showNFCReadCardIcon) {
        ...
        if (cardView == null) {
            ...
            Log.e(TAG, new Gson().toJson(mfError));
            return;
        } else {
    ...
    @NonNull
    @Override
    public void UpdateSession(@NonNull MFUpdateSessionRequest updateSessionRequest, @NonNull IMFCallBack promise) {
        MFSDK.INSTANCE.updateSession(updateSessionRequest, (MFResult<MFUpdateSessionResponse> result) -> {
            if (result instanceof MFResult.Success) {
                MFUpdateSessionResponse response = ((MFResult.Success<MFUpdateSessionResponse>) result).getResponse();
                OnSuccess(promise, response);
            } else if (result instanceof MFResult.Fail) {
                MFError mfError = ((MFResult.Fail) result).getError();
                OnError(promise, mfError);
    ...
    @NonNull
    @Override
    public void SetupGooglePayTokenHelper(@NonNull GooglePayRequest googlePayRequest, @NonNull IMFCallBack promise) {
        MFGooglePayButton googlePayButton = googlePayButtonFactory.mfGooglePayButton;
        if (googlePayButton == null) {
            MFError mfError = new MFError("010", "googlePayButton must be initialized");
            OnError(promise, mfError);
        ...
        mfGooglePayTokenHelper.setGooglePayButton(googlePayButton);
        googlePayButton.setVisibility(View.VISIBLE);
    }
    ...
    private void OnGPaySessionUpdated(String sessionId) {
        mfListener.OnSessionUpdated(sessionId);
    }
